            ...
            <div class="head bg-primary p-3 flex items-center justify-between text-white">
                <div class="flex items-center gap-2">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" stroke="white"
                        stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="-scale-x-100 transform">
                        <line x1="19" y1="12" x2="5" y2="12"></line>
                        <polyline points="12 19 5 12 12 5"></polyline>
                    </svg>
                    <input v-if="!changeStructure" type="text" :placeholder="$t('content.search')" v-model="store.searchText"
                        @keyup="searchFunc" class="bg-transparent placeholder:text-white text-white outline-none"
                        name="" id="">
                </div>
                <IconClose @click="store.showModalFilter = false" color="#fff"></IconClose>
            </div>
            <div class="h-[600px] md:h-[800px]  overflow-y-auto">
                <div v-if="!changeStructure">
                    <div @click="navTabs = 1" class="px-3 py-2 mt-8 border border-[#ccc] rounded-xl mx-3">
                        <span class="text-[#949393]"> {{ store.cityObj.name != "" ? store.cityObj.name : $t('content.selectedLocation') }} </span>
                    </div>
                    <div v-if="navTabs == 1" class=" overflow-y-auto px-3 mt-5">
        
                        <div class="flex flex-col gap-2">
                            <div v-if="store.pendingSearch" class="flex items-center flex-col gap-3 justify-center mt-10">
                                <span class="text-[14px] text-gray"> {{ $t('loadingText') }} </span>
                                <div role="status">
                                    <IconLoader></IconLoader>
                                </div>
                            </div>
                            <div v-for="item in store.cities" @click="getCity(item), navTabs = 2 , openDatePicker() , store.checkMobileCitySearch = false"
                            ...
                            <div v-for="item in store.hotels" @click="getHotel(item), navTabs = 2 , openDatePicker() , store.checkMobileCitySearch = false"
                ...
                <div @click="openDatePicker() , navTabs = 2" class="px-3 py-2 mt-8 border border-[#ccc] rounded-xl mx-3">
                    <span class="text-[#949393]"> {{ store.searchObj.checkIn }} - {{ store.searchObj.checkOut }} </span>
                </div>
                <VueDatePicker  ref="datepicker" v-model="selectedDate" :range="true" :min-date="minDate"
                    :max-date="maxDate" :multi-calendars="currentWidth <= 700 ? false : true" auto-apply trim-weeks
                    :first-day-of-week="1" :locale="locale" :show="['month', 'day']" :enable-time-picker="false"
                    :day-names="locale === 'en' ? dayNamesEN : dayNamesAR" @update:model-value="onDateChange"
                    class="custom-date-picker border-none" />
    
                <div @click="navTabs = 3" class="px-3 py-2 mt-8 border border-[#ccc] rounded-xl mx-3">
                    <span class="text-[#949393]"> {{ store.totalRoomsLength }} {{
                        $t('content.bedrooms') }} , {{ store.totalAdults }} {{ $t('content.adults') }} , {{
                            store.totalChilds }} {{ $t('content.children') }} </span>
                </div>
               
                <div v-if="navTabs == 3">
                    <div class="mt-5 border-purple-50  border-[1px]  mx-2 rounded-[6px]">
                ...
                <div class="mx-3 mt-3">
                    <button v-if="store.hotelId && !changeStructure" @click="goToHotel() , store.showModalFilter = false " class="bg-primary py-2 rounded-lg text-white w-full"> {{changeStructure ? $t('content.editSearch') : $t('content.search_now') }}  </button>
                    <button v-else @click="submitSearch() , store.showModalFilter = false " class="bg-primary py-2 rounded-lg text-white w-full"> {{changeStructure ? $t('content.editSearch') : $t('content.search_now') }}  </button>
                </div>
            </div>
